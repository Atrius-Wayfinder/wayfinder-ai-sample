...
describe("buildSystemInstruction", () => {
  ...
  it("should include dynamic system instruction warnings on iterations 8-9", () => {
    ...
    const instruction8 = buildSystemInstruction(8, testVenueId);
    ...
    const instruction9 = buildSystemInstruction(9, testVenueId);
    ...
    const instruction10 = buildSystemInstruction(10, testVenueId);
    expect(instruction10).toContain("no iterations left");

    // Iteration 7 should NOT have warning
    const instruction7 = buildSystemInstruction(7, testVenueId);
  ...
  it("should include base system instruction on all iterations", () => {
    const instruction1 = buildSystemInstruction(1, testVenueId);
  ...
  it("should include location context when pinned location is set", () => {
    const instruction = buildSystemInstruction(1, testVenueId);
  ...
  it("should include the venue ID in the system instruction", () => {
    const instruction = buildSystemInstruction(1, "my-airport");
    expect(instruction).toContain("[my-airport]");
  });
});
